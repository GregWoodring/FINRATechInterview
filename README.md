...
I am not a designer, the design of the product list leaves much to be desired, however I have added CSS styling to ensure it is
legible. I have not made this application mobile responsive, though this could be done through the use of media queries if some of the other css regarding how the background scales was redone. 

I have used redux for state management in this project. This is undoubtably overkill for an application this size, however I wanted to demonstrate my ability to use it. I opted not to use redux-promise-middleware for this application due to the fact that there is nearly no latency when making local calls. In a production environment however I likely would want to do this, it would allow me to filter cached data in the browser while awaiting additional calls to the backend server, and it would allow me to easily display loading indicators to create a more responsive design. 

I opted to use express as a backend server framework primarily due to familiarity. This application could absolutely be written purely with the http library, but the express library makes it easier.
